...
import { getWeatherDataById } from '../db/weather.js';
    ...
    describe('GET /api/weather', () => {
        describe('With valid input', () => {
            ...
            it('all base parameters should have its expected type', () => {
                    ...
                    expect(response.body._id).to.be.a('number');
            });
            ...
            it('check if data were saved correctly to database', async () => {
                const obj = await getWeatherDataById(response.body._id);
                expect(obj).to.be.an('array');
                const result = await JSON.parse(obj[0].weatherData);
                expect(result).to.be.an('object');
                expect(result).to.have.all.keys('coordinates', 'temperature', 'wind', 'clouds', 'rain', 'humidity','snow', 'time', 'location');
                ...
                expect(obj[0].id).to.be.equal(response.body._id);
                });
    });
